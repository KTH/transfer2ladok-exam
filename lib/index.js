...
async function getLadokModules (courseId, token) {
  ...
  // Regular SIS ID. Eg. AA0000HT19
  const matchNormal = course.sis_course_id.match(/(.*)(VT|HT)(\d{2})\d/)

  // Examination SIS ID. Eg. AA0000_XXXX_2020-01-01
  const matchExam = course.sis_course_id.match(/(.+)_(.+)_(\d\d\d\d-\d\d-\d\d)/)

  if (matchExam) {
    const [, courseCode, examCode, examDate] = matchExam

    return kopps.getModule(courseCode, examCode, examDate)
  } else if (matchNormal) {
    const [, courseCode, term, year] = matchNormal
...
async function getGrades (courseId, assignmentId, moduleId, token) {
  ...
  if (course.sis_course_id.match(/(.*)(VT|HT)(\d{2})\d/)) {
    ladokGrades = await ladok.listGradeableResults(
      course.integration_id,
      moduleId
    )
  } else if (course.sis_course_id.match(/(.+)_(.+)_(\d\d\d\d-\d\d-\d\d)/)) {
    const sections = (await canvas.getSections(token, courseId)).filter(
      section => section.integration_id
    )

    if (sections.length > 2) {
      log.warn(
        `Course ${course.sis_course_id} not eligible for exporting grades. It has ${sections.length} sections`
      )
    }

    const section = sections.find(
      section => !section.integration_id.endsWith('_FUNKA')
    )

    ladokGrades = await ladok.listExamGradeableResults(section.integration_id)
  }
...
async function sendGradesToLadok (
  ...
  if (course.sis_course_id.match(/(.*)(VT|HT)(\d{2})\d/)) {
    gradeableResults = await ladok.listGradeableResults(
      course.integration_id,
      moduleId
    )
  } else if (course.sis_course_id.match(/(.+)_(.+)_(\d\d\d\d-\d\d-\d\d)/)) {
    ...
    gradeableResults = await ladok.listExamGradeableResults(
  ...
  const ladokLink = course.integration_id
    ? await ladok.getModuleLink(course.integration_id, moduleId)
    : ''
