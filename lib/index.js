...
async function getGrades (courseId, assignmentId, moduleId, token) {
  ...
  const canvasGrades = await canvas.getSubmissions(token, courseId, assignmentId)


  for (const canvasGrade of canvasGrades) {
    const ladokGrade = await ladok.getLetterGrade(ladokGrades, moduleId, canvasGrade.user.integration_id)
