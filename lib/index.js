...
async function sendGradesToLadok (
  ...
  for (const result of gradeableResults) {
    ...
    try {
      ...
      if (e.message.match(/Grade .* not found/)) {
        throw new ClientError(
          'ladok_error',
          'Incorrect grading schema used. Make sure that the assignment uses the same grading schema as the module in Ladok (A-Fx or P/F)'
        )
      } else {
        throw e
      }
