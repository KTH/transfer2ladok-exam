...
const { ExportError, ClientError } = require('../errors')
...
async function getGradeId (scaleId, letterGrade) {
  ...
  if (!scale) {
    throw new ClientError(
      'ladok_error',
      `Grading scale "${scaleId}" not found in Ladok`
    )
  }
...
async function sendDraft (draft) {
  try {
    ...
    if (err.body.Meddelande) {
      const err2 = new ClientError('ladok_error', err.body.Meddelande)
      log.warn(
        err.body.Meddelande,
        'Error creating or updating grades to Ladok'
      )
      throw err2
    } else {
      throw err
