...
async function sendDraft (draft) {
  ...
  if (create.Resultat.length > 0) {
    try {
      ...
      if (err.body.Meddelande) {
        log.warn(`Error when creating grades: ${err.body.Meddelande}`)
        throw new ExportError('ladok_error', err.body.Meddelande)
      } else {
        throw err
      }
    }
  ...
  if (update.Resultat.length > 0) {
    try {
      ...
      if (err.body.Meddelande) {
        log.warn(`Error when updating grades: ${err.body.Meddelande}`)
        throw new ExportError('ladok_error', err.body.Meddelande)
      } else {
        throw err
      }
