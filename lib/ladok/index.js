...
async function sendDraft (draft) {
  try {
    ...
    if (err.body.Meddelande) {
      const err2 = new ExportError('ladok_error', err.body.Meddelande)
      log.warn(err2, 'Error creating or updating grades to Ladok')
      throw err2
    } else {
      throw err
    }
...
function getLetterGrade (ladokResults, moduleId, studentId) {
  ...
  if (!studentResult) {
    return { letter: null, existsInLadok: false }
  }

  const underlag = getArbetsUnderlag(studentResult, moduleId)

  if (!underlag) {
    return { letter: null, existsInLadok: true }
  }

  return { letter: underlag.Betygsgradsobjekt.Kod, existsInLadok: true }
}
...
async function getModuleLink (courseLadokId, momentId) {
  ...
  const ladokLink = [
    ...
    `/gui/#/resultatflikar?vy=rapportera&valtKurstillfalle=${courseLadokId}`,
