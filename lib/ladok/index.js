...
async function listExamGradeableResults (examId) {
  ...
  const { body } = await ladokApi.get(
    `/resultat/aktivitetstillfallesmojlighet/filtrera?aktivitetstillfalleUID=${examId}&page=1&limit=400`
  )

  const sections = Array.from(
    new Set(body.Resultat.map(r => r.Rapporteringskontext.KurstillfalleUID))
  )
