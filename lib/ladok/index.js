...
async function listReportersRaw () {
  const { body } = await ladokApi.get(
    `/kataloginformation/behorighetsprofil/${
      process.env.LADOK_REPORTER_PROFILE_UID
    }/koppladeanvandare`
  )
    ...
    async setGrade (result, grade, examinationDate) {
      ...
      if (underlag && underlag.Betygsgrad === newLadokGrade) {
        log.info(
          `Student ${
            result.Student.Uid
          }. SAME GRADE. Before: ${newLadokGrade}. After: ${newLadokGrade}`
        )
        return
      }
      ...
      if (!underlag) {
        ...
        log.info(
          `Student ${
            result.Student.Uid
          }. Before: ---. After: ${newLadokGrade} ex.date ${examinationDate}`
        )
        ...
        log.info(
          `Student ${result.Student.Uid}. Before: ${
            underlag.Betygsgrad
          }. After: ${newLadokGrade} ex.date ${examinationDate}`
        )
        // Update
